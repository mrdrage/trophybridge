...
Application code must depend on the internal `PsnProvider` contract rather than directly on a concrete community library. `MockPsnProvider` exists today; M2 adds the production `psn-api` adapter and provider mapping.
...
M1 establishes the database schema that receives those normalized types. Internal entities use TrophyBridge UUIDs while PlayStation identifiers remain explicit provider attributes.
...
- **M2 PSN Provider**: map community `psn-api` payloads into TrophyBridge domain types and implement the real adapter.
